...
fifo_generator_0 fifo (
  ...
  .wr_en(s_axis_txd_tvalid & (~fifo_full)),    // input wire wr_en
...
always @(posedge clk)
    begin
        if(rst_inv)
            pkt_to_send <= 0;
        else if(((s_axis_txd_tvalid_reg == 0) && (s_axis_txd_tvalid == 1)) || ((s_axis_txd_tlast_reg ==1) && (s_axis_txd_tlast == 0) && (s_axis_txd_tvalid) == 1))
            pkt_to_send <= 1;
        else if(m_axis_txc_tvalid == 1)
            pkt_to_send <= 0;
        else
...
always @(posedge clk)
    begin
        if(rst_inv)
            m_axis_txc_tvalid <= 0;
        else if((pkt_to_send == 1) &&(m_axis_txd_tready == 0))
            m_axis_txc_tvalid <= 1;
        else if(cnt == 5)
            m_axis_txc_tvalid <= 0;
        else
