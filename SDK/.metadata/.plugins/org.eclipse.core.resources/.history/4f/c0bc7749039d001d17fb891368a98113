...
//Set one of the DACs that control the White Rabbit VCOsnetif
...
//Get ph data triggered by SW
int Read_SW_PH();
//Write baseline to FPGA memory
int Write_BL(u16 * ph_baseline_array);
...
int main()
{
	...
	*/
	...
	/******************************************************/
	// sci_pcb is implemented in hardware
	/*
	sci_pcb = udp_new();
	if (!sci_pcb) {
		xil_printf("Error creating PCB. Out of Memory\r\n");
...
recv_callback(void *arg, struct udp_pcb *tpcb,struct pbuf *p, struct ip4_addr *addr, u16_t port)
{
		...
		if ((byte0 & 0x7f) == 0x0c)
		{	
			xil_printf("Read SW PH\r\n");
			Status =Read_SW_PH();
			if(Status != 0) xil_printf("SW PH data can't be read\r\n");
		}
		if((byte0 & 0x7f) == 0x0d)
		{
			xil_printf("Write BL Data\r\n");
...
void Set_Acquisition(u16* dataptr)
{
	...
	//Get the value of slv_reg8- we need to preserve the mask bits
	regval = Xil_In32(MAROC_DC + 32) & 0x1ff;
#ifdef HW_GTV08
...
int PH_BL_Init(u16 * ph_baseline_array)
{
	...
	for (loopcount = 0; loopcount < 8; loopcount++)
	{
		...
		*/
		...
		for(bl_array_index=0;bl_array_index<256;bl_array_index++)
		{
		#define RESIDUAL_BL 10
...
int Write_BL(u16 *ph_baseline_array){
	xil_printf("ph_bl[0]=%d\r\n", ph_baseline_array[0]);
	xil_printf("ph_bl[1]=%d\r\n", ph_baseline_array[1]);
	BL_Write(ph_baseline_array);
	u16 bl_data_r[256];
	BL_Read(bl_data_r);
	char bl_state;
...
int UdpSendData(ip4_addr_t *dst_ip_ptr, u16 port, char *d)
{
	struct pbuf *arp_pbuf;
	int len = strlen(d);
	arp_pbuf = pbuf_alloc(PBUF_TRANSPORT, len, PBUF_RAM);
	if (!arp_pbuf) {
		xil_printf("error allocating pbuf to send\r\n");
		return -1;
	}
	char * arp_payload_ptr = arp_pbuf->payload;
	memcpy(arp_payload_ptr, d, len);
	struct udp_pcb *arp_pcb;
	/* create a udp socket for the arp data */
	arp_pcb = udp_new();
	if (!arp_pcb) {
		xil_printf("Error creating PCB. Out of Memory\r\n");
		return -2;
	}
	/* connect to the dst IP**************************/
	err_t err;
	err = udp_connect(arp_pcb, dst_ip_ptr, port);
	if (err != ERR_OK)
		xil_printf("error on udp_connect: %x\n\r", err);
	err = udp_send(arp_pcb, arp_pbuf);
	if (err != ERR_OK) {
		xil_printf("Error on command udp_send: %d\r\n", err);
		pbuf_free(arp_pbuf);
		udp_remove(arp_pcb);
